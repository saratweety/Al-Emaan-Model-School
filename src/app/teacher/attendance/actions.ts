...
export async function getAttendanceForDate(
  studentIds: string[],
  date: string
): Promise<{ statuses: Record<string, AttendanceStatus>; error: string | null }> {
  const { supabase, userId, error } = await requireTeacher();
  if (error || !userId) return { statuses: {}, error: error ?? "Not authorized." };

  if (studentIds.length === 0) return { statuses: {}, error: null };
  ...
  if (fetchError) return { statuses: {}, error: fetchError.message };
